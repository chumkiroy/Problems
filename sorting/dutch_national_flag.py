    ...
    while i <= k:
        if balls[i] == 'R':
            if j == i:
                ...
                cj = balls[j]
                ci = balls[i]
                #balls = balls[:j]+ ci +balls[j+1:]
                #balls = balls[:i]+ cj+balls[i+1:]
                balls[j], balls[i] = balls[i], balls[j]
                j+=1
                i-=1
        elif balls[i] == 'B' and i != k :
            while balls[k] == 'B'and k > 0:
                k -= 1
            if i < k:
                ck = balls[k]
                ci = balls[i]
                #balls = balls[:k]+ ci +balls[k+1:]
                #balls = balls[:i]+ ck +balls[i+1:]
                balls[k], balls[i] = balls[i], balls[k]
                k -= 1
                i -=1
        i += 1
    return ''.join(balls)
