...
Sample Input 2: efabcdhefhabcdiefi
Sample Output 2: abcd
...
def  LRS(iString):
    ...
    for i in range(len(iString)):
        suffix.append(iString[i:])
    
    suffix = sorted(suffix)
    lrs = ''
    
    for i in range(0, len(suffix)-1):
        x = longestCommonPrefix(suffix[i], suffix[i+1])
        if len(x) > len(lrs):
            lrs = x
    return lrs

print LRS("efabcdhefhabcdiefi")
