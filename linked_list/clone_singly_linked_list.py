...
def cloneLinkedList(head):
    ...
    temp = head
    while temp:
        clone = SinglyLinkedListNode(temp.data)
        clone_map[temp] = clone
        temp = temp.next
    temp = head
    while temp:
        clone = clone_map[temp]
        if temp.next:
            clone.next = clone_map[temp.next]
        if temp.randomPointer:
            clone.randomPointer = clone_map[temp.randomPointer]
        temp = temp.next
